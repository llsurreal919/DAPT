...
class Sablock(nn.Module):
    def __init__(self, bottleneck_size):
        ...
        self.avg_pool = torch.nn.AvgPool1d(kernel_size=128)

        self.fc1 = nn.Linear(bottleneck_size+1, bottleneck_size+1//2)
        self.fc2 = nn.Linear(bottleneck_size+1//2, bottleneck_size)

    def forward(self, x, SNR):
        x_pooled = self.avg_pool(x.permute(0, 2, 1)).permute(0, 2, 1)
        snr_expanded = SNR.unsqueeze(2)
        x_cat = torch.cat([x_pooled, snr_expanded], dim=2)

        x_fc1 = self.fc1(x_cat)
        x_ReLu = F.relu(x_fc1)
        x_fc2 =  self.fc2(x_ReLu)
        weight = torch.sigmoid(x_fc2)

        W = weight.repeat(1, x.shape[1], 1)
