...
class CRNet(torch.nn.Module):
    ...
    def forward(self, point_cloud, feat=None, snr=None):
        ...
        p1_pred, up_feat1 = self.upsampling_stage_1(point_cloud, feat, snr)         # p1_pred(B, C3, N512)  # up_feat1(B, C128, N512)
        p2_pred, up_feat2 = self.upsampling_stage_2(p1_pred, up_feat1, snr)         # p2_pred(B, C3, N2048)  # up_feat1(B, C128, N2048)
...
class SubNetwork(nn.Module):
    ...
    def __init__(self, up_ratio=2):
        ...
        self.deconv = nn.ConvTranspose1d(128, 128, 1, stride=1)
    ...
    def forward(self, points, feat, snr):
        ...
        out_feature = self.modulation(feat, snr)            # out_feature(B, C128, N128)
...
class Transformer_extractor(nn.Module):
    ...
    def forward(self, points, feat):
        ...
        feature_2 = torch.cat([feat, global_feature.repeat((1, 1, feat.size(2)))], 1)
...
class Upsampling_unit(nn.Module):
    ...
    def forward(self, point_feat, points):
        deconved_feat = self.deconv_branch(self.mlp_1(point_feat))  # (B, 128, 128)->(B, 32, 128)->(B, 128, 512)
