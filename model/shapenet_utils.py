...
def sample_and_group_all_gt(xyz, points):
    ...
    Input:
        ...
        points: input points data, [B, N, D]
    Return:
        new_xyz: sampled points position data, [B, N, 3]
        new_points: sampled points data, [B, 1, N, 3+D]
    """
    device = xyz.device
    B, N, C = xyz.shape
...
class TransitionDown(nn.Module):
    ...
    def forward(self, xyz, feature):
        ...
        p1_trans = p.transpose(1, 2).contiguous() # (B, 3, N)
        idx = pointops.furthestsampling(p, M)
        p2 = pointops.gathering(p1_trans, idx).transpose(1, 2).contiguous()
               
        n_x, _ = self.grouper(xyz=p, new_xyz=p2, features=x.contiguous()) # (B, 3 + C_in, M, K)
...
class CoordinateUpsamplingModule_0(nn.Module):
    def __init__(self, ratio, channels=256, radius=0.05):
        super(CoordinateUpsamplingModule_0, self).__init__()
        self.ratio = ratio
        self.radius = radius
...
class SA_Layer(nn.Module):
    ...
    def forward(self, x, xyz):
        x = x + xyz #[B32,C256,N128]
        x_q = self.q_conv(x).permute(0, 2, 1)#[B32,C64,N128].permute(0, 2, 1)

        x_k = self.k_conv(x)#[B32,C64,N128]
        x_v = self.v_conv(x)#[B32,C256,N128]

        energy = torch.bmm(x_q, x_k) #QK^T

        attention = self.softmax(energy)
        attention = attention / (1e-9 + attention.sum(dim=1, keepdim=True)) #列
