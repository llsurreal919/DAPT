...
class Model(nn.Module):
    def __init__(self, normal_channel=False, bottleneck_size=200, recon_points=2048, channel_name='AWGN',
        ...
        self.sa1 = PointNetSetAbstraction(npoint=512, radius=0.2, nsample=16, in_channel=3 + 3, mlp=[64, 64, 128], group_all=False)
        self.attn1 = TransformerBlock(128, 128, 16)
        self.sa2 = PointNetSetAbstraction(npoint=128, radius=0.4, nsample=16, in_channel=128 + 3, mlp=[128, 128, bottleneck_size], group_all=False)
        self.attn2 = TransformerBlock(bottleneck_size, bottleneck_size, 16)
    ...
    def forward(self, xyz, band, training = True, simu_name = 'single_coderate_multipe_snr'):
        ...
        l1_xyz, l1_feature = self.sa1(l0_xyz, feats)
        gt_downsample = l1_xyz.transpose(1, 2)
