...
class CRNet(torch.nn.Module):
    ...
    def forward(self, point_cloud, feat=None):
        point_cloud = point_cloud.contiguous() 
        p1_pred, up_feat1 = self.upsampling_stage_1(point_cloud, feat)
        p2_pred, up_feat2 = self.upsampling_stage_2(p1_pred, up_feat1)
        p3_pred, _ = self.refinement_stage(p2_pred, up_feat2) 
...
class SubNetwork(nn.Module):
    ...
    def forward(self, points, feat):
        point_feat = self.feature_extractor(points, feat)  # (B, C256, N128)
        up_feat, duplicated_point = self.up_unit(point_feat, points) # (B, C256, N512)# (B, 3, 512)
...
class Transformer(nn.Module):
    ...
    def forward(self, x, pos):
        ...
        key = pointops.grouping(key, idx_knn)  # b, dim, n, n_knn
        qk_rel = query.reshape((b, -1, n, 1)) - key
        pos_rel = pos.reshape((b, -1, n, 1)) - pointops.grouping(pos.contiguous(), idx_knn)  # b, 3, n, n_knn
